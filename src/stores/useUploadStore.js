...
export const useUploadStore = defineStore('upload', {
  ...
  actions: {
    ...
    async uploadFile ({ file, cardId, spaceId, boxId }) {
      ...
      const currentSpaceId = spaceId || spaceStore.id
      ...
      if (currentSpaceId) {
        formData.append('requestSpaceId', currentSpaceId)
      }
      ...
      return new Promise((resolve, reject) => {
        ...
        request.upload.onprogress = async (event) => {
          ...
          const percentCompleteDisplay = Math.min(Math.floor(percentComplete), 99)
          ...
          // end
          if (percentComplete >= 100 && request.readyState === -1) {
            const complete = {
              cardId,
              spaceId,
              boxId,
        ...
        request.onload = () => {
          ...
          if (!isSuccess) {
            this.removePendingUpload({ cardId, spaceId, boxId })
            cardStore.clearCardNameUploadPlaceholder(cardId)
          ...
          const complete = {
            ...
            spaceId,
            boxId,
            url: response.url || `${consts.cdnHost}/${key}`,
            fileName
          }
          console.info('Upload completed', complete)
          globalStore.triggerUploadComplete(complete)
          this.removePendingUpload({ cardId, spaceId, boxId })
          resolve(response)
        }
        ...
        if (userStore.apiKey) {
          request.setRequestHeader('Authorization', userStore.apiKey)
        }
