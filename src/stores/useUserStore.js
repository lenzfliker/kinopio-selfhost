  ...
  actions: {
    ...
    async restoreUserAssociatedData () {
      ...
      try {
        ...
        if (!consts.isSelfHosted()) {
          requests.push(apiStore.getUserGroups())
        }
        const [favoriteSpaces, favoriteUsers, favoriteColors, hiddenSpaces, tags, groups] = await Promise.all(requests)
        if (favoriteUsers) {
