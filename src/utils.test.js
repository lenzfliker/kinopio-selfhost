...
describe('urlIsSpace', () => {
  it('detects self-host space URLs on the current origin', () => {
    const url = `${window.location.origin}/space-abcdefghijk1mn0pqrstu`
    expect(utils.urlIsSpace(url)).toBe(true)
  })

  it('detects self-host card links on the current origin', () => {
    const url = `${window.location.origin}/space-abcdefghijk1mn0pqrstu/card123`
    expect(utils.urlIsSpace(url)).toBe(true)
  })
})
